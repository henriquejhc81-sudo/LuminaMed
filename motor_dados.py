...
def carregar_banco_medicamentos():
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            elif 'ANALGÉSICO' in classe or 'N2B' in atc or 'ANTIPIRÉTICO' in classe:
                sintomas = "dor, febre, dor de cabeça, dor no corpo, dipirona, paracetamol"
            elif 'ANTI-INFLAMATÓRIO' in classe or 'M1A' in atc or 'M2A' in atc:
                sintomas = "dor, inflamação, inchaço, dor muscular, dor articular, garganta inflamada"
            elif 'ANTI-HISTAMÍNICO' in classe or 'R6A' in atc or 'ANTIALÉRGICO' in classe:
            ...
            elif 'ANTIÁCIDO' in classe or 'A2A' in atc or 'BOMBA DE PRÓTONS' in classe or 'A2B' in atc:
                sintomas = "azia, queimação, refluxo, dor de estômago, gastrite"
            elif 'BRONCODILATADOR' in classe or 'R3A' in atc or 'ASMA' in classe:
            ...
            elif 'ANTIFÚNGICO' in classe or 'ANTIMICÓTICO' in classe or 'D1A' in atc or 'J2A' in atc:
                sintomas = "fungo, micose, coceira, frieira, cândida, pano branco"
            elif 'ANTIVIRAL' in classe or 'J5' in atc:
                sintomas = "vírus, herpes, gripe viral, infecção viral"
            elif 'DIURÉTICO' in classe or 'C3A' in atc:
                sintomas = "pressão alta, retenção de líquido, inchaço, edema"
            elif 'ANTI-HIPERTENSIVO' in classe or 'C2' in atc or 'C9' in atc or 'BETABLOQUEADOR' in classe:
                sintomas = "pressão alta, hipertensão, pressão arterial"
            elif 'ANTIDIABÉTICO' in classe or 'A10' in atc or 'INSULINA' in classe:
                sintomas = "diabetes, glicose alta, açúcar no sangue, hiperglicemia"
            elif 'RELAXANTE MUSCULAR' in classe or 'M3' in atc:
                sintomas = "tensão muscular, dor nas costas, torcicolo, espasmo muscular"
            elif 'ANTIDEPRESSIVO' in classe or 'N6A' in atc:
                sintomas = "depressão, ansiedade, pânico, tristeza, insônia"
            elif 'ANTIVERTIGINOSO' in classe or 'N7C' in atc:
